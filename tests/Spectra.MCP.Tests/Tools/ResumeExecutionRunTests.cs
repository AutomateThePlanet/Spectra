...
public class ResumeExecutionRunTests : IAsyncDisposable
{
    private readonly string _testDir;
    private readonly ExecutionDb _db;
    private readonly ExecutionEngine _engine;
    private readonly ResumeExecutionRunTool _tool;
    private readonly TestIndexEntry[] _testEntries;

    public ResumeExecutionRunTests()
    {
        ...
        _tool = new ResumeExecutionRunTool(_engine, runRepo);
    ...
    [Fact]
    public async Task Execute_PausedRun_ReturnsRunningStatus()
    {
    ...
    public async Task Execute_ReturnsNextTest()
    {
        ...
        var nextTest = response.GetProperty("data").GetProperty("next_test");
        Assert.True(nextTest.TryGetProperty("test_handle", out _));
        Assert.True(nextTest.TryGetProperty("test_id", out _));
        Assert.True(nextTest.TryGetProperty("title", out _));
    }
    ...
    [Fact]
    public async Task Execute_RunningRun_ReturnsError()
    {
        var (run, _) = await _engine.StartRunAsync("checkout", _testEntries);
        var parameters = JsonDocument.Parse($$$"""{"run_id": "{{{run.RunId}}}"}""").RootElement;
        ...
        Assert.Equal("Running", response.GetProperty("current_run_status").GetString());
    }

    [Fact]
    public async Task Execute_PreservesProgressAfterResume()
    {
    ...
    [Fact]
    public async Task Execute_AllTestsComplete_ReturnsFinalizeAction()
    {
        var (run, queue) = await _engine.StartRunAsync("checkout", _testEntries);
        ...
        // Pause (this works even with all tests complete but not finalized)
        await _engine.PauseRunAsync(run.RunId);
        var parameters = JsonDocument.Parse($$$"""{"run_id": "{{{run.RunId}}}"}""").RootElement;
