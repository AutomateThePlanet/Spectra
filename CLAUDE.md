...
## Recent Changes
- **077-nonstop-batch-verbs (v2.4.0)**: Closes the class of non-stop seam breaches confirmed during the v2.3.0 run. **FR1 (`ingest-verdict --suite --all`):** `IngestVerdictCommand.cs` gains `--suite` + `--all` batch mode — enumerates `.spectra/verdicts/critic-verdict-*.json`, filters by suite via `_index.json`, classifies each with `VerdictIngestor.Classify()`, emits `{grounded, partial, hallucinated, errors}` summary. Replaces per-test `--from` shell loops. **FR2 (`ingest-update {suite} --all`):** `IngestUpdateCommand.cs` gains `--all` batch mode — enumerates `.spectra/updates/{suite}/updated-{id}.json` staging files, ingests each via `UpdatedTestIngestor.IngestAsync()`, emits `{written, skipped_no_original, errors}`. Defines new staging convention `.spectra/updates/{suite}/updated-{id}.json`. `--test-id` becomes optional (not required when `--all`). **FR3 (skill: batch calls):** `spectra-generate.md` Step 8a replaces per-test `ingest-verdict --from` (removed from subagent description; main flow reads verdict file with Read tool after subagent returns) with a post-loop `ingest-verdict --suite --all` call before `ingest-grounding --all`. Step 8b restructured: repair loop writes patches to `.spectra/updates/{suite}/updated-{id}.json` (Write tool only per entry), then batch `ingest-update {suite} --all`, then re-critic loop (reads verdict files), then batch `ingest-verdict --suite --all`. The existing `ingest-grounding --all` and written==0 guard remain UNCHANGED (hard regression gate). **FR4 (manifest consumption):** Step 8b gains an explicit instruction — use the Read tool on a spilled manifest file, iterate in-context, never pipe to `python -c` / `jq` / interpreter, never accept "scripting for all projects" allowlist option. **FR5 (general preamble):** A NON-STOP CONTRACT section is added before Step 7 naming all four prohibited behaviors (shell loops, interpreter pipes, manual `.md` editing, verdict field rewrites) and requiring STOP+report when no single `spectra` call covers a step. The per-seam guards are specific instances of this general rule. **FR6 (docs):** `cli-reference.md` and `usage.md` updated with new batch verb docs and updated Repair Batch section. +2 test files (10 new tests: 5 × FR1, 5 × FR2). Core 568 / CLI 1276 / Execution 228.
- **075-fix-arithmetic-index-skill (v2.3.0)**: Closes three root causes found in the unit-converter audit. **FR1 (critic arithmetic mandate):** `critic-verification.md` gains an explicit `## Arithmetic Verification` section — when an expected result is a computed value (unit conversion, formula output, scientific-notation magnitude, derived constant), the critic MUST independently compute and compare; a wrong number is NOT `grounded` even if the underlying principle is documented. Worked example: TC-107 (`1×10⁻⁹ km → 1E-9 nm`, correct `1000 nm`). **FR2 (index-writer path fix, two lines):** `IngestTestsCommand.LoadExistingTestsAsync:153` and `IngestUpdateCommand.LoadExistingTestsAsync:166` changed `Path.GetRelativePath(testsPath, file)` → `Path.GetRelativePath(suitePath, file)` — bare filename on every round (first and subsequent), consistent with `GeneratedTestIngestor.ParseTestCase:237`. The three 073 consumer fixes (`AuditGroundingHandler:91`, `CompileRepairBatchCommand:95`, `IngestGroundingCommand:115`) are UNCHANGED (hard regression gate). **FR3 (skill failure-branch guard):** `spectra-generate.md` now checks the post-8a `ingest-grounding --all` JSON result against kept-grounded count: `written==0 && kept-grounded>0` → STOP+report, no 8b entry, no manual edits; partial write → warning; restates NON-STOP CONTRACT at the branch point. **FR4 (poisoned-index regeneration):** in-session cleanup step — `spectra docs index --suites unit-converter` after the writer fix to heal the unit-converter `_index.json`. **FR5 (arithmetic sweep):** in-session step — re-critic the 56 grounded tests under the new mandate for TC-107-class errors. **FR6 (docs):** `grounding-verification.md` gains Arithmetic Verification section; `cli-reference.md` notes bare-filename convention; `usage.md` repair-batch section gains the written-vs-kept-grounded check block and NON-STOP CONTRACT restatement. +4 test files (2 new regression guard classes × 2 tests each). Core 568 / CLI 1266 / Execution 228.
