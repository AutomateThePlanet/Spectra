# Agent Prompt Contract: SPECTRA Execution Agent
...
| Purpose | Path |
|---------|------|
| Copilot Chat Agent | `.github/agents/spectra-execution.agent.md` |
| Copilot CLI Skill | `.github/skills/spectra-execution/SKILL.md` |
...
description: string   # Required. One-line description for tool discovery
```
...
When `spectra init` is run:

1. Check if `.github/agents/spectra-execution.agent.md` exists
   - If exists and `--force` not provided: Skip, log "Agent file exists, skipping"
   - If exists and `--force` provided: Overwrite
   - If not exists: Create with bundled content

2. Check if `.github/skills/spectra-execution/SKILL.md` exists
   - Same logic as above

3. Create parent directories if needed:
   - `.github/agents/`
   - `.github/skills/spectra-execution/`

---

## Versioning

Agent prompt version is tracked in the prompt content itself:

```markdown
<!-- SPECTRA Execution Agent v1.0.0 -->
```

Updates to prompt content increment version following semver:
