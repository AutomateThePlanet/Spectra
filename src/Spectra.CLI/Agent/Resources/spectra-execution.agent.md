...
1. Call `finalize_execution_run` before ending
2. Show final summary: total, passed, failed, blocked, skipped
3. Show the report file paths
4. **Open the HTML report automatically**: show preview .execution/reports/{html_filename}
5. Mention any tests that were not executed
6. Offer to export results or log bugs for failures
...
When the user asks for coverage, dashboard, validation, or other non-execution tasks, run them via `runInTerminal`. **NEVER create files manually — always use the CLI.**
...
#### Tool call 1: runInTerminal
```
spectra ai analyze --extract-criteria --output-format json --verbosity quiet
```
#### Tool call 2: awaitTerminal
#### Tool call 3: terminalLastCommand

Parse the JSON result. Show `newCount` new acceptance criteria extracted, `duplicatesSkipped` skipped, `totalInFile` total. List individual criteria from the `criteria` array.
...
Report: "Opened the existing dashboard. Say 'regenerate dashboard' to rebuild with latest data."
