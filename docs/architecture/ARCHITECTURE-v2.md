...
Copilot `.github/agents/` translates to **two distinct Claude Code mechanisms that must not be conflated:**

- **Slash commands** (`.claude/commands/`) — prompt templates injected into the *current* session. Generation and the authoring verbs. No isolation.
- **Subagent** (`.claude/agents/`) — its own context window, tool restrictions, returns a summary without polluting the parent. **This is the critic.** Delegated **explicitly as a mandatory step**, never left to auto-selection (the critic *always* runs).
...
## Open questions — verify before any spec (do not assume)

1. **Subagent mechanics** — auto vs. explicit invocation, input passing, whether gen→critic can chain in one user action.
2. **Screenshot / image paste** in Claude Code VS Code — does it work such that an MCP tool can consume it? Central to the fail/blocked flow (`save_clipboard_screenshot`).
3. **Permission / confirmation model** — replaces Copilot's `askQuestion` / `askForConfirmation` dialog tools.
4. **Billing edge** — do interactive MCP tool calls stay on subscription, or is there a programmatic edge?
...
1. **Doc verification** of the four open Claude Code mechanics against current official docs.
2. **Investigation MD** on the real generation path in code — where the critic is injected today, what the current handoff looks like — grounded in file + line evidence.
