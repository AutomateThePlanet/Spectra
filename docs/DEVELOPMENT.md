...
├── docs/
│   ├── _index.md                # Document index (auto-generated)
│   └── features/
...
The document index is automatically refreshed (incremental) before every `spectra ai generate` run. This ensures the AI agent always has an up-to-date view of documentation without manual intervention.

During `spectra init`, an initial full index build is performed if documentation files exist.
...
If `doc_index` is not set, defaults to `{local_dir}/_index.md` (typically `docs/_index.md`).

## Key Types

| Type | Location | Purpose |
|------|----------|---------|
| `DocumentIndex` | Spectra.Core/Models | Top-level container with entries and totals |
| `DocumentIndexEntry` | Spectra.Core/Models | Per-document metadata (path, title, sections, entities, hash) |
| `SectionSummary` | Spectra.Core/Models | Per-section heading, level, and 200-char summary |
| `DocumentIndexExtractor` | Spectra.Core/Parsing | Extracts metadata from Markdown files |
| `DocumentIndexWriter` | Spectra.Core/Index | Renders index to Markdown format |
| `DocumentIndexReader` | Spectra.Core/Index | Parses Markdown index back to model (full or hash-only) |
| `DocumentIndexService` | Spectra.CLI/Source | Orchestrates incremental/full index builds |
