...
**That server was removed entirely in Spec 070.** There is nothing to `dotnet tool install`,
...
it's dead weight — safe to remove. `spectra init` no longer writes either.

## How it works now

1. **You ask, in Claude Code:** *"Run the checkout suite"* or *"Execute the high-priority auth
   tests."*
2. **The execution skill/agent orchestrates**, it doesn't drive a per-test loop in chat:
...
4. **The agent reads current state from `spectra run status`** (the SQLite store) when you ask
   ...
   source of truth.
5. **Finalize** — `spectra run finalize` generates JSON, Markdown, and HTML reports under
   `.execution/reports/`.

SQLite (`.execution/spectra.db`) is the single source of truth throughout. The console is a view
and a write-back caller, never a store — refreshing or reopening it loses nothing, and a
short-lived `spectra run` CLI invocation reconstructs the exact same state losslessly (Spec 064).
...
`source_refs` documentation directly with the Read tool — no external service or configuration
...
Reports carry per-test enrichment sourced from the test case itself — `priority`, `tags`,
