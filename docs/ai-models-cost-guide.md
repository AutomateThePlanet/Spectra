...
**That entire billing surface is gone.** v2.4.0 deleted the GitHub Copilot SDK, `ai.providers`,
...
updates. There is nothing left in `spectra.config.json` to route requests to a provider, because
legacy configs that still carry `ai.providers`/`ai.critic` load cleanly, but the keys do nothing.
...
There is no separate "SPECTRA AI cost" anymore, since it is exactly your normal Claude Code usage, and
...
since SPECTRA has no visibility into or influence over those numbers.
...
currently using, so pick it the same way you'd pick a model for any other piece of work (`/model`,
...
source docs), so it doesn't grow your main session's context window, but each call still counts as
normal Claude Code usage. The critic's model is a static `model:` field in that agent file (shipped
as `claude-sonnet-4-6`); there is no `spectra.config.json` knob for it anymore, so edit the agent
...
|------|----------|-----------|
| Haiku | Straightforward CRUD docs, light re-verification, fast iteration | Shallower behavior analysis, meaning fewer implicit edge cases surfaced |
...
because ARCHITECTURE-v2 §32 found a same-family critic (Sonnet verifying Sonnet-authored tests)
reads as *more* useful in practice, since it shares the same standard of evidence. The critic
...
- `generation_batch_size` (default 30) sets how many tests are requested per generation turn. Larger batches
...
- `generation_timeout_minutes` and `analysis_timeout_minutes` bound the deterministic
...
SPECTRA can no longer report per-call token usage the way it did through v1, since it isn't the process
...
surfaces, whether your plan's usage indicator or the API/Console token dashboard if you're on
...
authenticate, since you're already authenticated as whatever account is running your Claude Code
